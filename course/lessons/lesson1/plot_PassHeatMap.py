...
for idx in match_ids:
    ...
    for period in [1, 2]:
        ...
        #keep only danger passes
        danger_passes2_period = passes.loc[pass_to_shot]
        #concatenate dataframe with a previous one to keep danger passes from the whole tournament
        danger_passes2 = pd.concat([danger_passes2, danger_passes2_period], ignore_index = True)
...
fig, ax = pitch.grid(grid_height=0.9, title_height=0.06, axis=False,
                     endnote_height=0.04, title_space=0, endnote_space=0)
#scatter the location on the pitch
pitch.scatter(danger_passes2.x, danger_passes2.y, s=100, color='blue', edgecolors='grey', linewidth=1, alpha=0.2, ax=ax["pitch"])
#uncomment it to plot arrows
...
for idx in match_ids:
    ...
    for period in [1, 2]:
        ...
        #keep only danger passes
        danger_passes3_period = passes.loc[pass_to_shot]
        #concatenate dataframe with a previous one to keep danger passes from the whole tournament
        danger_passes3 = pd.concat([danger_passes3, danger_passes3_period], ignore_index = True)

print(danger_passes3.player_name.value_counts())
#%%
player_most_involved = danger_passes3.player_name.value_counts().idxmax()

danger_passes3 = danger_passes3[danger_passes3.player_name == player_most_involved]
#%%
# Making a heat map
# ----------------------------
# To make a heat map, first, we draw a pitch. Then we calculate the number of passes in each bin using *bin_statistic* method.
...
print(danger_passes3.player_name.unique())
