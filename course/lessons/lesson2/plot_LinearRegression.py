...
#%%
# Use 'Age' and 'Min' columns for the analysis
laliga_df_clean = laliga_df[['Age', 'Min']].dropna()

# Step 1: Fit a simple linear regression model (Min ~ Age)
linear_model = smf.ols(formula='Min ~ Age', data=laliga_df_clean).fit()

# Step 2: Add a cubic term for age (Min ~ Age + Age^2 + Age^3)
laliga_df_clean['Age_squared'] = laliga_df_clean['Age'] ** 2
laliga_df_clean['Age_cubed'] = laliga_df_clean['Age'] ** 3

cubic_model = smf.ols(formula='Min ~ Age + Age_squared + Age_cubed', data=laliga_df_clean).fit()

# Step 3: Compare the summaries of the models
linear_summary = linear_model.summary()
cubic_summary = cubic_model.summary()

linear_summary, cubic_summary
